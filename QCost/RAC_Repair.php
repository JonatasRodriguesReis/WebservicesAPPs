...
$result = $conn->query("SELECT day(max(data)) as dia FROM qcost WHERE modelo LIKE 'RAC' AND custo LIKE 'Repair' AND year(data) = $ano and MONTH(data) = $mes");
$outp = array();
...
for($i=1; $i<=$dia_max; $i++){
	$data = "$ano-$mes-$i";
	$result = $conn->query("SELECT valor FROM qcost WHERE modelo LIKE 'RAC' AND custo LIKE 'Repair' AND data = '$data'");
	$outp = array();
    $outp = $result->fetch_all(MYSQLI_ASSOC);
	$entrou = false;
    foreach ($outp as $value) {
