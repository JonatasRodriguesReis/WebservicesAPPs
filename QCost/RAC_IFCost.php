...
$result = $conn->query("SELECT day(max(data)) as dia FROM qcost WHERE modelo LIKE 'RAC' AND (custo LIKE 'Scrap' OR custo LIKE 'Nowork' OR custo LIKE 'Rework' OR custo LIKE 'Line Repair') AND year(data) = $ano and MONTH(data) = $mes");
$outp = array();
...
for($i=1; $i<=$dia_max; $i++){
	...
	$result = $conn->query("SELECT valor, custo FROM qcost WHERE modelo LIKE 'RAC' AND (custo LIKE 'Scrap' OR custo LIKE 'Nowork' OR custo LIKE 'Rework' OR custo LIKE 'Line Repair') AND data = '$data'");
	$outp = array();
    $outp = $result->fetch_all(MYSQLI_ASSOC);
	$entrou = false;
    foreach ($outp as $value) {
